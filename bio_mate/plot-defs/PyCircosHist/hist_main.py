...
def main(configfile):
    ...
    if config["plot_settings"]["show_hist"] and \
        ...
        for num, inputdata in enumerate(p.all_data["hist"]):
            print(config["plot_settings"]["hist_split_value"])
            if num > 3: continue 
            p.d.add(p.PlotData(
                inner_r   = config["general"][f"hist_{num+1}_r"][0],
                outer_r   = config["general"][f"hist_{num+1}_r"][1],
                plottype  = "hist",
                inputdataname = inputdata[0],
                inputdata = inputdata[1],
                show_bg   = True,
                bg_grid_num = 4,
                limit_max_value = config["plot_settings"]["hist_limit_value"][1], #注意后一个是max
                limit_min_value = config["plot_settings"]["hist_limit_value"][0], #注意前第一个是min
