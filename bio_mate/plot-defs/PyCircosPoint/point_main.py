...
def main(configfile):
    ...
    if config["plot_settings"]["show_point"] and \
        ...
        for num, inputdata in enumerate(p.all_data["point"]):
            if num > 3: continue 
            p.d.add(p.PlotData(
                inner_r   = config["general"][f"point_{num+1}_r"][0],
                outer_r   = config["general"][f"point_{num+1}_r"][1],
                plottype  = "point",
                inputdataname = inputdata[0],
                inputdata = inputdata[1],
                show_bg   = True,
                point_size = config["plot_settings"]["point_size"],
                point_fmt  = {
