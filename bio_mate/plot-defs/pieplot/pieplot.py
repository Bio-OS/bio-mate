...
def plot_pieplot(config_file, output_file):
    # 读取配置文件
    ...
    Explode=config['columns']['Explode']


    #绘图参数设置
    fontsize=int(config['plot_settings']['fontsize'])
    color=config['plot_settings']['color']
    autopct=config['plot_settings']['autopct']
    shadow=config['plot_settings']['shadow']
    startangle=int(config['plot_settings']['startangle'])
...
if __name__ == "__main__":
    ...
    # 根据命令行参数执行相应的操作
    if args.action == 'plot':
        plot_pieplot(args.config, args.output)
    else:
