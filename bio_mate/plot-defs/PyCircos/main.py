...
def main(configfile):
    ...
    if config["plot_settings"]["show_hist"] and \
        len(p.all_data.get("hist", [])) > 0:
        for inputdata in p.all_data["hist"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r   = plot_r[0],
                outer_r   = plot_r[1],
                plottype  = "hist",
                ...
                show_bg   = True,
                bg_grid_num = 4,
                limit_max_value = config["plot_settings"]["hist_limit_value"][0],
                limit_min_value = config["plot_settings"]["hist_limit_value"][1],
                split_value = config["plot_settings"]["hist_split_value"],
                hist_fmt  = {
                    "strokeWidth": config["plot_settings"]["hist_strokeWidth"],
                    },
                show_label = config["plot_settings"]["show_hist_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["hist_label_fontSize"],
                    "fillColor": mytoColor(config["plot_settings"]["hist_label_fillColor"]),
                    },
                ))
    if config["plot_settings"]["show_line"] and \
        len(p.all_data.get("line", [])) > 0:
        for inputdata in p.all_data["line"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r   = plot_r[0],
                outer_r   = plot_r[1],
                ...
                bg_grid_num = 4,
                line_fmt = {
                    "strokeWidth": config["plot_settings"]["line_strokeWidth"],
                    "strokeColor": mytoColor(config["plot_settings"]["line_strokeColor"]),
                    },
                show_label = config["plot_settings"]["show_line_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["line_label_fontSize"],
                    "fillColor": mytoColor(config["plot_settings"]["line_label_fillColor"]),
                    },
            ))
    if config["plot_settings"]["show_line_area"] and \
        len(p.all_data.get("line_area", [])) > 0:        
        for inputdata in p.all_data["line_area"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r = plot_r[0],
                outer_r = plot_r[1],
                ...
                inputdata = inputdata[1],
                line_fmt = {
                    "strokeWidth": config["plot_settings"]["line_area_strokeWidth"],
                    "strokeColor": mytoColor(config["plot_settings"]["line_area_strokeColor"]),
                    "fillColor":   mytoColor(config["plot_settings"]["line_area_fillColor"]),
                    },
                show_label = config["plot_settings"]["show_line_area_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["line_area_label_fontSize"],
                    "fillColor": mytoColor(config["plot_settings"]["line_area_label_fillColor"]),
                    },
            ))
    if config["plot_settings"]["show_segment"] and \
        len(p.all_data.get("segment", [])) > 0:
        for inputdata in p.all_data["segment"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r = plot_r[0],
                outer_r = plot_r[1],
                ...
                inputdata = inputdata[1],
                line_fmt = {
                    "strokeWidth": config["plot_settings"]["segment_strokeWidth"],
                    },
                show_label = config["plot_settings"]["show_segment_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["segment_label_fontSize"],
                    "fillColor": mytoColor(config["plot_settings"]["segment_label_fillColor"]),
                    },
            ))
    if config["plot_settings"]["show_point"] and \
        len(p.all_data.get("point", [])) > 0:
        for inputdata in p.all_data["point"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r = plot_r[0],
                outer_r = plot_r[1],
                ...
                inputdata = inputdata[1],
                point_fmt = {
                    "strokeWidth": config["plot_settings"]["point_strokeWidth"],
                    "strokeColor": mytoColor(config["plot_settings"]["point_strokeColor"]),
                    },
                show_label = config["plot_settings"]["show_point_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["point_label_fontSize"],
                    "fillColor": mytoColor(config["plot_settings"]["point_label_fillColor"]),
                    },
            ))
    if config["plot_settings"]["show_link"] and \
        len(p.all_data.get("link", [])) > 0:
        for inputdata in p.all_data["link"]:
            plot_r = [float(pr) for pr in inputdata[2].strip().split(",")]
            p.d.add(p.PlotData(
                inner_r = plot_r[0],
                outer_r = plot_r[1],
