...
def main(configfile):
    ...
    if config["plot_settings"]["show_line"] and \
        ...
        for num, inputdata in enumerate(p.all_data["line"]):
            if num > 3: continue 
            p.d.add(p.PlotData(
                inner_r   = config["general"][f"line_inner"][num],
                outer_r   = config["general"][f"line_outer"][num],
                plottype  = "line",
                inputdataname = inputdata[0],
                inputdata = inputdata[1],
                show_bg = True, #(True if num==0 else False),
                limit_max_value = p.all_data_max["line"],
                limit_min_value = p.all_data_min["line"],
                line_fmt  = {
                    "strokeWidth": config["plot_settings"][f"line_strokeWidth"][num],
                    "strokeColor": mytoColor(config["plot_settings"][f"line_strokeColor"][num]),
                    },
                show_label = config["plot_settings"]["show_line_label"],
                data_label_fmt = {
                    "fontSize":  config["plot_settings"]["line_label_fontSize"][num],
                    "fillColor": mytoColor(config["plot_settings"]["line_label_fillColor"][num]),
                    },
                ))
    if True: #肯定要绘制标题
