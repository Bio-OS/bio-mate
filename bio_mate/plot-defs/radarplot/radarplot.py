...
def plot_radarplot(config_file, output_file):
    # 读取配置文件
    ...
    plt.yticks([10,50,100], ["10","50","100"], color="grey", size=fontsize)
    ...
    for i in data.index:
        ...
        values += values[:1]
        ax.plot(angles, values, linewidth=linewidth, linestyle='solid', color=color[i],label=f"{data.loc[i,group]}")
        ax.fill(angles, values,  alpha=alpha,color=color[i])
...
if __name__ == "__main__":
    ...
    # 根据命令行参数执行相应的操作
    if args.action == 'plot':
        plot_radarplot(args.config, args.output)
    else:
